...
public class EUMInstrumentationHook implements ISpecialHook {
		...
		if (performCheck && configurationValid) {
			try {
				...
				if (agentURLMatcher.matches()) {
					...
					sendScript(res, agentBuilder.buildJsFile(modules));
					return true;
				}
				return false;
	...
	public final void initConfig(IConfigurationStorage configurationStorage) {
		try {
			...
			tags.append(base).append(JSAgentModule.BEACON_SUB_PATH).append("\""); // NOPMD
			tags.append(", relevancyThreshold : ").append(endUserMonitoringConfig.getRelevancyThreshold());
			tags.append(", allowListenerInstrumentation : ").append(endUserMonitoringConfig.isListenerInstrumentationAllowed());
