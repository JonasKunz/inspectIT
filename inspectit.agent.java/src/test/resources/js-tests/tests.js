...
QUnit.test("Soft-Reference Freeing", function( assert ) {
	...
	//go just before the limit
	for(var i = 1; i < MAX_ACTIVE_REFERENCES; i++) {
		inspectIT.createEUMElement("test").setParent(parent);
	}
	//make sure it is not cleared yet
	assert.ok(parent === first.getParent(), "Reference not cleared before the limit is reached.")
	//add another element to exceed the limit
	inspectIT.createEUMElement("test").setParent(parent);	
	assert.ok(null === first.getParent(), "Reference cleared after the limit is reached.")
});
