...
(function(){
		...
		function checkBeaconSendTimeout() {
			if ( dataToSend.length > 0) {
				...
				inspectIT.instrumentation.runWithout(function() {
					...
					//send after the minimum of TIME_WINDOW ms of inactivity or of MAX_TIME_WINDOW ms since the last beacon was sent
					...
					if( totalElapsedMS >= MAX_TIME_WINDOW || elapsedSinceLastSendRequestMS >= TIME_WINDOW) {
						...
						if(!awaitingResponse) {
							//only send one beacon at a time - as soon as the last beacon is finished it will attempt to call checkBeaconSendTimeout()
							forceBeaconSend();							
						}
					} else {
				...
				prototypeToInstrument.addEventListener = function(type, callback, optionsOrCapture) {
					...
					if( "_inspectIT_instrumentedCallback" in callback) {
						...
						instrumentedCallback = function() {
							...
							//recursive iterator
							//when this function is invoked, it either calls the next isntrumentation or the actual callback if all instrumentations were executed
	...
	var traceBuilder = (function() {
		 ...
		 * to correctly lcoate them within teh existing trace.
		 */
		var traceObservers = [];
	 ...
	 * A "use" is defined as calling refernce.setTarget.
	 * 
	...
	function createLimitedReferencePool(maxActiveReferences) {
		...
		Reference.prototype.setTarget = function(target) {
			//first step - remove from the lsit
